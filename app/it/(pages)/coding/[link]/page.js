...
export default async function PostPage({ params }) {
  ...
  // Get related posts (other Coding posts)
  const allCodingPosts = getMetadata('코딩');
  const relatedPosts = allCodingPosts
...
export async function generateMetadata({ params, searchParams }) {
  const resolvedParams = await params;
  return generateITPageMetadata(resolvedParams.link, 'coding');
}