...
export default async function PostPage({ params }) {
  ...
  // Get related posts (other tech company posts)
  const allTechCompanyPosts = getMetadata('테크회사들');
  const relatedPosts = allTechCompanyPosts
  ...
  const breadcrumbItems = [
    { name: 'IT', href: '/it' },
    { name: '테크회사들', href: '/it/techcompany' },
...
export async function generateMetadata({ params, searchParams }) {
  ...
  return { 
    ...
    alternates: {
        canonical: 'https://moyahug.com/it/techcompany/' + resolvedParams.link,
    }
  };
}