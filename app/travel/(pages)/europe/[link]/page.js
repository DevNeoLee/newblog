...
export default async function PostPage({ params }) {
  ...
  // Get related posts (other Europe posts)
  const allEuropePosts = getMetadata('유럽');
  const relatedPosts = allEuropePosts
    .filter(p => p.link !== link)
    .slice(0, 3)
    .map(p => {
      // Get actual content for reading time calculation
      const postContent = getPostContent(p.link, 'europe');
      return {
        ...p,
        section: 'travel',
        category: 'europe',
        href: `/travel/europe/${p.link}`,
...
export async function generateMetadata({ params, searchParams }) {
  const resolvedParams = await params;
  const link = resolvedParams.link;
  const details = getPostContent(link, 'europe');
  ...
  const structuredData = generateArticleStructuredData(details, link, 'europe', 'travel');

  // Generate standardized metadata
  return generateArticleMetadata(details, link, 'europe', structuredData);
}